...
class AnswerController extends Controller
{
    ...
    public function show($id)
    {
        return Admin::content(function (Content $content) use ($id) {
            ...
            $content->body(Admin::show(Answer::findOrFail($id), function (Show $show) {

                $show->id('编号');
                $show->sort('排序');
                $show->title('标题');
                $show->investigation_id('所属问题');
...
//            });
    ...
    protected function form()
    {
        return Admin::form(Answer::class, function (Form $form) {
            ...
            $form->number('sort','排序');
            $form->select('investigation_id','所属问题')->options('/admin/AjaxAnswer')->rules('required');
